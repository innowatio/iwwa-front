...
export function getDateRangesCompare ({period, dateOne}) {
    if (period.key === "years" || period.key === "months") {
        ...
        const startTwo = period.key === "years" ?
            moment.utc(dateOne).startOf("day").subtract({weeks: moment.utc().weeksInYear() + 1}).weekday(1).valueOf() :
            moment.utc(dateOne).startOf("day").subtract({weeks: 6}).weekday(1).valueOf();
